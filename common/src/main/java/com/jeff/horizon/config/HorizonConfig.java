...
public class HorizonConfig {
    ...
    public static class KeyBindingImpl {
        ...
        public final KeyMapping toggleNuit = new KeyMapping("key.horizon.toggle", InputConstants.Type.KEYSYM, -1, "category.nuit");
        public final KeyMapping toggleSkyboxDebugHud = new KeyMapping("key.horizon.toggle.debug_hud", InputConstants.Type.KEYSYM, -1, "category.nuit");
        ...
        public void tick(Minecraft client) {
            while (this.toggleNuit.consumeClick()) {
                ...
                if (SkyboxManager.getInstance().isEnabled()) {
                    player.displayClientMessage(Component.translatable("horizon.message.enabled"), false);
                } else {
                    player.displayClientMessage(Component.translatable("horizon.message.disabled"), false);
                }
            }
