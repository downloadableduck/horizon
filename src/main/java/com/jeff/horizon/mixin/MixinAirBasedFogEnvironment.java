...
public class MixinAirBasedFogEnvironment {
    @Redirect(method = "getBaseColor", remap = false, at = @At(value = "INVOKE", target = "Lnet/minecraft/client/multiplayer/ClientLevel;getTimeOfDay(F)F", remap = false))
    ...
    @Redirect(method = "getBaseColor", remap = false, at = @At(value = "INVOKE", target = "Lnet/minecraft/client/multiplayer/ClientLevel;getSunAngle(F)F", remap = false))
    ...
    @ModifyConstant(method = "getBaseColor", remap = false, slice = @Slice(from = @At(value = "INVOKE", target = "Lnet/minecraft/util/CubicSampler;gaussianSampleVec3(Lnet/minecraft/world/phys/Vec3;Lnet/minecraft/util/CubicSampler$Vec3Fetcher;)Lnet/minecraft/world/phys/Vec3;", remap = false)), constant = @Constant(intValue = 4, ordinal = 0))
    private int nuit$renderSkyColor(int original) {
        ...
        if (skyboxManager.isEnabled() && skybox instanceof HorizonSkybox horizonSkybox && !horizonSkybox.getProperties().renderSunSkyTint()) {
     ...
     */
    @Inject(method = "getBaseColor", at = @At(value = "TAIL"), cancellable = true, remap = false)
