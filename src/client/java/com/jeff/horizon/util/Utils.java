...
public class Utils {
    ...
    public static double calculateRotation(double rotationSpeed, boolean isSkyboxRotation, ClientLevel world) {
        if (rotationSpeed != 0F) {
            ...
            if (isSkyboxRotation) {
                ...
                return 360D * (Minecraft.getInstance().level.getOverworldClockTime() % 24000);
            }
        } else {
            return 0D;
        }
    ...
    public static RGB alphaBlendFogColors(List<Skybox> skyboxList, RGB initialFogColor) {
        ...
        for (Skybox skybox : skyboxList) {
            if (skybox.isActive() && skybox instanceof HorizonSkybox horizonSkybox && horizonSkybox.getProperties().fog().isModifyColors()) {
                final RGB fogColor = horizonSkybox.getProperties().fog();
                destination = fogColor.blend(destination, horizonSkybox.getAlpha());
            }
        }
    ...
    public static float alphaBlendFogDensity(List<Skybox> skyboxList, float initialFogDensity) {
        ...
        for (Skybox skybox : skyboxList) {
            if (skybox.isActive() && skybox instanceof HorizonSkybox horizonSkybox && horizonSkybox.getProperties().fog().isModifyDensity()) {
                float sourceAlphaInv = 1.0F - horizonSkybox.getAlpha();
                destination = (horizonSkybox.getProperties().fog().getDensity() * horizonSkybox.getAlpha()) + (destination * sourceAlphaInv);
            }
        }
    ...
    public static <T> T loadService(Class<T> clazz) {
        ...
        HorizonClient.getLogger().debug("Loaded {} for service {}", loadedService, clazz);
