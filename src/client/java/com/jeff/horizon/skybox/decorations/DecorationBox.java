...
public class DecorationBox extends AbstractSkybox {
    ...
    private static final Function<ResourceLocation, RenderType> CELESTIAL = Util.memoize((resourceLocation) -> create("celestial", 1536, false, false, RenderPipelines.CELESTIAL, RenderType.CompositeState.builder().setTextureState(new RenderStateShard.TextureStateShard(resourceLocation, false)).createCompositeState(false)));

    public RenderType celestial(ResourceLocation resourceLocation) {
    ...
    public static RenderType.CompositeRenderType create(String string, int i, boolean bl, boolean bl2, RenderPipeline renderPipeline, RenderType.CompositeState compositeState) {
    ...
    private void renderSun(MultiBufferSource multiBufferSource, PoseStack poseStack) {
        VertexConsumer vertexConsumer = multiBufferSource.getBuffer(this.celestial(this.sunTexture));
